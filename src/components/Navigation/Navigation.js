    ...
    <nav className={`navigation ${isBurgerMenuOpen && 'navigation_isOpen navigation_type_pseudo-style'} ${isLoggedIn && isBurgerMenuOpen && 'navigation_isOpenAndLoggedIn '} ${isSaveArticlesPageIsOpen && 'navigation_type_save-articles'} `} >
      ...
      <ul className={`navigation__list ${isBurgerMenuOpen && 'navigation__list_isOpen'}`} >
        <li className='navigation__item'>
          <button onClick={() => handleHomeBtnClick()} type='button' className={`navigation__link_type_home-page navigation__link ${isSaveArticlesPageIsOpen && !isBurgerMenuOpen && 'navigation__link_color_black'} ${isLoggedIn && 'navigation__link_type_logged-in'}`} >Home</button>
        </li>
        {isLoggedIn ?
          <li children='navigation__item'>
            <button onClick={() => { handleSavedArticlesBtnClick() }} type='button' className={`navigation__link_type_saved-articles navigation__link ${isSaveArticlesPageIsOpen && !isBurgerMenuOpen && 'navigation__link_color_black navigation__link_borderB_black'} ${isLoggedIn && 'navigation__link_type_logged-in'}`} >Saved articles</button>
          </li> : ''}
        <li className='navigation__item'>
          <button onClick={() => onClick()} className={`navigation__button ${isSaveArticlesPageIsOpen && !isBurgerMenuOpen && 'navigation__button_color_black'} ${isLoggedIn && 'navigation__button_logged-in'}`} type='button'>{isLoggedIn ? 'Elise' : 'Sign in'}{isLoggedIn && <img src={!isBurgerMenuOpen && isSaveArticlesPageIsOpen ? buttonLogoBlack : buttonLogoWhite}></img>} </button>
        </li>
      </ul>
